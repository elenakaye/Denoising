...
def generate_patches(isDebug=False):
    ...
    count = 0
    src_dir = "./data/TrainMRI"
    output_file = "img_clean_pats"
    if args.target_train == "target":
        src_dir = "./data/TargetMRI"
        output_file = "img_target_pats"
    ...
    for i in xrange(len(filepaths)):
        ...
        print "generating image: %s" % filepaths[i]
        for s in xrange(len(scales)):
            newsize = (int(img.size[0] * scales[s]),
                       int(img.size[1] * scales[s]))
            # print newsize
    ...
    np.save(os.path.join(args.save_dir, output_file), inputs)
