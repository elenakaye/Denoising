...
def generate_patches(isDebug=False):
    ...
    sigma = 25.0
    count = 0
    src_dir = "./data/TargetMRI"
    noisy_img_dir = "./data/TrainMRI"
    ...
    filepaths_noisy = glob.glob(noisy_img_dir + '/*.png')
    filepaths_noisy.sort()
    ...
    count = 0
    random_mode_vals = []
    # generate patches for target (clean) images
    for i in xrange(len(filepaths)):
    ...
    count = 0
    # generate patches for noisy data
    for i in xrange(len(filepaths_noisy)):
        img = Image.open(filepaths_noisy[i]).convert('L')
        print "generating noisy image: %s" % filepaths_noisy[i]
        for s in xrange(len(scales)):
            newsize = (int(img.size[0] * scales[s]),
                       int(img.size[1] * scales[s]))
            # print newsize
            ...
            if s == 0: # save only once
                ...
                #save_images(os.path.join(noisy_img_dir, 'noisy_%s' % (filepaths_noisy[i].split("/")[3])),
                           # noisyimg)
            ...
            for j in xrange(DATA_AUG_TIMES):
                ...
                for x in range(0 + args.step, im_h - args.pat_size, args.stride):
                    for y in range(0 + args.step, im_w - args.pat_size, args.stride):
                        ...
                        #print "img %s :: scale %d, x_val: %d, y_val: %d, random_mode_n %d" % (filepaths_noisy[i], s, x,y,random_mode_n)
                        inputs[count, :, :, :] = data_augmentation(img_s[x:x + args.pat_size, y:y + args.pat_size, :], random_mode_n)
                        count += 1
    # pad the batch
    if count < numPatches:
        to_pad = numPatches - count
